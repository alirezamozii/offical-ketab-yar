...
import { Home, LayoutDashboard, RotateCcw } from 'lucide-react'
import Link from 'next/link'
import { useEffect, useRef } from 'react'

/**
 * /dashboard error boundary — catches errors on the dashboard page.
 * The dashboard aggregates a lot of data (XP, streak, heatmap,
 * achievements, recommendations), so an error here is often from one
 * of the underlying fetches failing. The primary action is retry.
 */
export default function DashboardError({
  ...
  useEffect(() => {
    console.error('[ketab-yar] /dashboard error boundary:', {
  ...
  return (
    ...
    >
      ...
      <span className="flex h-20 w-20 items-center justify-center rounded-3xl bg-gold-500/15 text-gold-600 shadow-md-warm dark:text-gold-400">
        <LayoutDashboard className="h-10 w-10" aria-hidden="true" />
      </span>
      ...
      <p className="mt-3 max-w-md text-pretty text-muted-foreground">
        در نمایش آمار و پیشرفت شما خطایی رخ داد. داده‌های شما ذخیره شده‌اند و
        با تلاش دوباره نمایش داده می‌شوند. اگر مشکل ادامه داشت، به خانه
        برگردید.
      </p>
      ...
      <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
        <Button
          ...
          variant="glow"
          size="lg"
          aria-label="تلاش دوباره برای بارگذاری داشبورد"
        >
