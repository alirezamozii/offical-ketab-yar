...
import { BookX, Library, RotateCcw } from 'lucide-react'
import Link from 'next/link'
import { useEffect, useRef } from 'react'

/**
 * /books error boundary — catches errors on the book detail page
 * (`/books/[slug]`) and any future route under `/books/*` that doesn't
 * have its own `error.tsx`. (The reader at `/books/read/[slug]` is a
 * full-screen layout and intentionally has no boundary here — it has
 * its own loading skeleton and inline error handling inside the reader
 * client component.)
 *
 * Contextual message: "کتاب بارگذاری نشد" with a recovery action that
 * sends the user back to the library (where they can pick another book).
 */
export default function BooksError({
  ...
  useEffect(() => {
    console.error('[ketab-yar] /books error boundary:', {
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 max-w-md text-pretty text-muted-foreground">
        در نمایش این کتاب مشکلی پیش آمد. شاید آدرس تغییر کرده باشد یا
        بارگذاری کامل انجام نشده باشد. می‌توانید دوباره تلاش کنید یا به
        کتابخانه برگردید.
      </p>
