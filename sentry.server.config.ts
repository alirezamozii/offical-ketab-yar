 ...
 *
 * Loaded from `src/instrumentation.ts` ONLY when `process.env.NEXT_RUNTIME === 'nodejs'`.
 * Next.js evaluates the instrumentation hook ONCE at boot, before any
 * request is served — so this is the right place to install server-side
 * error monitoring.
 *
 * Configuration mirrors the client side:
 *  • DSN from `SENTRY_DSN` (server-side env var, NOT exposed to the browser).
 *  • Disabled in dev unless explicitly enabled.
 *  • `tracesSampleRate: 0.1` for production performance sampling.
 *  • `beforeSend` scrubs PII (emails, IPs, cookies, auth headers) from
 *    outbound events so the deployment stays GDPR-compliant by default.
 *
...
Sentry.init({
  ...
  // Server-side: capture 100 % of errors (volume is low; we want full coverage).
  // Keep performance traces at 10 %.
  beforeSend(event) {
