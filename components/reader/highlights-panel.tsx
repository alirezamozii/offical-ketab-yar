...
export function HighlightsPanel({
  ...
  return (
    <motion.div
      ...
      className={cn(
        'fixed inset-y-0 right-0 z-[110] flex w-96 flex-col overflow-hidden border-l shadow-2xl',
        theme === 'light' && 'border-gold-200 bg-gradient-to-br from-gold-50 to-amber-50',
        theme === 'sepia' && 'border-amber-300 bg-gradient-to-br from-amber-100 to-amber-50',
        theme === 'dark' && 'border-gold-700 bg-gradient-to-br from-[#0f0e0c] to-[#1a1612]'
      )}
    >
      {/* Header */}
      <div className="flex-shrink-0 border-b border-gold-300 p-6 dark:border-gold-700">
        <div className="mb-2 flex items-center justify-between">
          <h2 className="text-xl font-bold">هایلایت‌ها</h2>
          <Button variant="ghost" size="sm" onClick={onClose} aria-label="بستن" title="بستن">
            <X className="h-5 w-5" />
          </Button>
        </div>
      ...
      <AnimatePresence>
        {showUndo && previousPage !== null && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="absolute left-4 right-4 top-24 z-10"
          >
            <Button
              onClick={handleUndo}
              className="w-full bg-gold-600 text-white shadow-lg hover:bg-gold-700"
              size="sm"
            >
      ...
      <div className="flex-1 space-y-2 overflow-y-auto p-4">
        {sortedPages.length === 0 ? (
          <div className="py-12 text-center text-muted-foreground">
            ...
            <p className="mt-2 text-sm">متن را انتخاب کنید و رنگ دلخواه را بزنید</p>
          </div>
        ) : (
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="flex w-full items-center justify-between p-4 transition-colors hover:bg-gold-100 dark:hover:bg-gold-900/30"
                >
                  ...
                  {isCurrentPage && (
                    <span className="rounded-full bg-gold-600 px-2 py-1 text-xs text-white">
                      صفحه فعلی
                    </span>
                  )}
                </button>
                ...
                <AnimatePresence>
                  {isExpanded && (
                    ...
                    >
                      <div className="space-y-2 bg-white/50 p-2 dark:bg-black/20">
                        {pageHighlights.map(highlight => (
                          <motion.div
                            ...
                            className={cn(
                              'group relative rounded-lg border-l-4 p-3',
                              getColorClass(highlight.color)
                            )}
                          >
                            <p className="mb-3 pr-2 text-sm leading-relaxed">"{highlight.text}"</p>
