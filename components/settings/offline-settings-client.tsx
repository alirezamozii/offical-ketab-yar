...
import { deleteBookOffline, getOfflineBooks, getStorageUsage } from '@/lib/pwa/offline-storage'
import { motion } from 'framer-motion'
import { AlertCircle, BookOpen, HardDrive, RefreshCw, Trash2 } from 'lucide-react'
...
export function OfflineSettingsClient() {
  ...
  const loadData = async () => {
    setIsLoading(true)
    try {
      const [books, storage] = await Promise.all([getOfflineBooks(), getStorageUsage()])
  ...
  return (
    <div className="container max-w-4xl space-y-6 py-8">
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">مدیریت کتاب‌های دانلود شده و حافظه دستگاه</p>
      </div>
      ...
      {queueSize > 0 && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Button onClick={syncNow} disabled={isSyncing || !navigator.onLine} className="w-full">
              {isSyncing ? (
                <>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="py-12 text-center">
              ...
              <p className="mt-4 text-sm text-muted-foreground">هنوز کتابی دانلود نکرده‌اید</p>
              ...
              {offlineBooks.map((book, index) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="truncate font-semibold">{book.title.fa || book.title.en}</h3>
                    <p className="truncate text-sm text-muted-foreground">{book.author.name}</p>
                    <p className="text-xs text-muted-foreground">
                      دانلود شده: {new Date(book.downloadedAt).toLocaleDateString('fa-IR')}
                    </p>
                  </div>
                  ...
                  <Button
                    ...
                    aria-label={`حذف کتاب ${book.title.fa || book.title.en} از حافظه آفلاین`}
                  >
