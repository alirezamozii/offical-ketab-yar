...
export function FindFriends({ userId }: FindFriendsProps) {
  ...
  async function handleSendRequest(friendId: string) {
    ...
    try {
      ...
      setResults(results.map(u => (u.id === friendId ? { ...u, requestSent: true } : u)))
    } catch (error: any) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        ...
        />
        <Button onClick={handleSearch} disabled={loading} aria-label="جستجو">
          ...
          {results.map(user => (
            <div key={user.id} className="flex items-center gap-4 rounded-lg border p-4">
              ...
              <Button
                ...
                aria-label={user.requestSent ? 'درخواست ارسال شد' : 'ارسال درخواست دوستی'}
              >
                {sendingTo === user.id ? (
