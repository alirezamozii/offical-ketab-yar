...
export function VocabularyManager({ userId }: VocabularyManagerProps) {
  ...
  // ⚡ Bolt: Derived state optimization - useMemo avoids unnecessary re-renders
  // and simplifies state management compared to manually syncing via useEffect
  const filteredWords = useMemo(
    () => (activeTab === 'all' ? words : words.filter(word => word.status === activeTab)),
    [activeTab, words]
  )
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          next_review_at: new Date(
            Date.now() +
              (newStatus === 'mastered' ? 7 : newStatus === 'reviewing' ? 3 : 1) *
                24 *
                60 *
                60 *
                1000
          ...
          <TabsContent value={activeTab}>
            {isLoading ? (
              <div className="py-8 text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground">در حال بارگذاری واژگان...</p>
              </div>
              ...
              <div className="space-y-4">
                ...
                {words.filter(w => w.status === 'learning' || w.status === 'reviewing').length >=
                  5 && (
                  <div className="mt-8">
                    <AdvancedFlashcardSystem
                      words={words
                        .filter(w => w.status === 'learning' || w.status === 'reviewing')
                        .map(mapToFlashcardWord)}
