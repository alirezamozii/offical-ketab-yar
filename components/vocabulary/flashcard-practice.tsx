...
export function FlashcardPractice({ bookId, mode }: FlashcardPracticeProps) {
  ...
  const fetchDefinition = async (word: string) => {
    ...
    try {
      ...
      const synonyms = Array.from(
        new Set(meanings?.definitions?.flatMap((d: Definition) => d.synonyms || []) || [])
      ).slice(0, 5) as string[]

      const antonyms = Array.from(
        new Set(meanings?.definitions?.flatMap((d: Definition) => d.antonyms || []) || [])
      ).slice(0, 5) as string[]
      ...
      <span>
        {parts.map((part, i) =>
          regex.test(part) ? (
            <span
              key={i}
              className="rounded bg-beige-300 px-1 font-bold text-gold-800 dark:bg-gold-500/30 dark:text-gold-100"
            >
              {part}
            </span>
          ) : (
      ...
      <div className="container mx-auto p-8" dir="rtl">
        <Card>
          <CardContent className="pt-6 text-center">
            ...
            <p className="mb-2 text-lg">کلمه‌ای برای تمرین وجود ندارد</p>
  ...
  return (
    <div className="container mx-auto flex min-h-screen flex-col p-4 md:p-8" dir="rtl">
      {/* Header */}
      <div className="mb-6">
      ...
      <div className="flex flex-1 items-center justify-center">
        <AnimatePresence mode="wait">
          ...
          >
            <Card className="border-gold/30 border-2 shadow-2xl">
                          ...
                          <>
                            ...
                            <div className="rounded-lg border border-blue-500/20 bg-blue-500/10 p-4">
                              <div className="mb-2 flex items-center justify-between">
                                ...
                                <button
                                  ...
                                  className="flex items-center gap-1 text-xs text-primary transition-colors hover:text-primary/80 disabled:opacity-50"
                                >
                                  {loadingTranslation ? (
                              ...
                              <p className="mb-2 text-base">{dictionaryData.definition}</p>

                              {translatedDefinition && (
                                <motion.div
                                  initial={{ opacity: 0, y: -10 }}
                                  animate={{ opacity: 1, y: 0 }}
                                  className="mt-3 border-t border-blue-500/20 pt-3"
                                >
                                  <p className="mb-1 text-sm text-muted-foreground">ترجمه:</p>
                            ...
                            {dictionaryData.synonyms.length > 0 && (
                              <div className="rounded-lg border border-green-500/20 bg-green-500/10 p-4">
                                <p className="mb-2 text-sm text-muted-foreground">مترادف‌ها:</p>
                            ...
                            {dictionaryData.antonyms.length > 0 && (
                              <div className="rounded-lg border border-red-500/20 bg-red-500/10 p-4">
                                <p className="mb-2 text-sm text-muted-foreground">متضادها:</p>
                                <div className="flex flex-wrap gap-2">
                                ...
                                {[...Array(7)].map((_, i) => (
                                  <div
                                    ...
                                    className={`h-2 w-6 rounded-full ${
                                      i < currentWord.mastery_level ? 'bg-gold-600' : 'bg-muted'
                                    }`}
                                  />
                                ))}
                              </div>
                  ...
                  {!showAnswer ? (
                    <>
                      <Button
                        ...
                        className="border-red-500/50 px-8 hover:bg-red-500/10 hover:text-red-700"
                      >
